...
from brave.api.service.analysis_node_service import find_running_analysis_node
...
class DockerExecutorV2(JobExecutor):
    ...
    def recover_running_containers(self,containers):
        ...
        with get_engine().begin() as conn:  
            running_jobs = find_running_analysis_node(conn)  # 异步获取所有运行中任务
    ...
    def _sync_submit_job(self, job: AnalysisExecuterModal) -> str:
        ...
        if find_container["envionment"]:
            ...
            envionment = envionment.replace("$SCRIPT_FILE", job.script_path)
        ...
        if job.run_id.startswith("server-") or job.run_id.startswith("retry-") or job.run_id.startswith("tools-") or job.run_id.startswith("nserver-"):
            ...
            if labels:
                ...
                labels = labels.replace("$CONTAINER_NAME",job.run_id)
        ...
        #             }})
        entrypoint = None
        if job.run_id.startswith("job-") or job.run_id.startswith("node-"):
            entrypoint="bash"
        try:
